...
import type {AuthorDetails, DbBook} from '../../types';
    ...
    const fetchAuthorData = async () => {
      ...
      try {
        const [authorRes, worksRes] = await Promise.all([
          fetch(`/api/authors/${authorConfig.id}`),
          fetch(`/api/books?author=${authorConfig.id}&limit=12`)
        ]);

        if (!authorRes.ok) throw new Error('No se ha encontrado el autor');
        if (!worksRes.ok) throw new Error('No se pudieron obtener los libros del autor.');
