...
const apiService = {
    ...
    getUserFavorites: (): Promise<DbBook[]> => fetch('/api/favorites', { headers: apiService.getAuthHeaders() }).then(res => res.json()),
    getUserBooks: (): Promise<UserBook[]> => fetch('/api/reading', { headers: apiService.getAuthHeaders() }).then(res => res.json()),
    startReadingBook: (bookId: number): Promise<Response> => fetch('/api/reading/start', { method: 'POST', headers: apiService.getAuthHeaders(), body: JSON.stringify({ bookId }) }),
    updateBookProgress: (bookId: number, progress: number, status: string): Promise<Response> => fetch('/api/reading/update', { method: 'PUT', headers: apiService.getAuthHeaders(), body: JSON.stringify({ bookId, progress, status }) }),
    rateBook: (bookId: number, rating: number): Promise<Response> => fetch('/api/reading/rate', { method: 'PUT', headers: apiService.getAuthHeaders(), body: JSON.stringify({ bookId, rating }) }),
    getUserCollections: (): Promise<Collection[]> => fetch('/api/collections', { headers: apiService.getAuthHeaders() }).then(res => res.json()),
    addBookToCollection: (collectionId: number, bookId: number): Promise<Response> => fetch('/api/collections/add-book', { method: 'POST', headers: apiService.getAuthHeaders(), body: JSON.stringify({ collectionId, bookId }) }),
    removeBookFromCollection: (collectionId: number, bookId: number): Promise<Response> => fetch('/api/collections/remove-book', { method: 'POST', headers: apiService.getAuthHeaders(), body: JSON.stringify({ collectionId, bookId }) }),
};
...
export const UserDataProvider = ({ children }: { children: ReactNode }) => {
    ...
    const value: UserDataContextType = { favoriteBooks, userBooks, favoritesCollectionId, isLoading, toggleFavorite, updateBookStatus, updateBookRating };
